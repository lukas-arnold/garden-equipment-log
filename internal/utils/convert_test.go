...
func TestConvertBytesToEquipmentStorage(t *testing.T) {
	...
	_, err := ConvertBytesToEquipmentStorage(data)
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestConvertBytesToEquipmentStorageInvalid(t *testing.T) {
	_, err := ConvertBytesToEquipmentStorage([]byte("not json"))
	if err == nil {
...
func TestParseDateTime(t *testing.T) {
	got, err := ParseDateTime("2026-01-01T12:30")
	if err != nil {
		t.Fatal(err)
	}

	expected := time.Date(2026, time.January, 1, 12, 30, 0, 0, time.UTC)
...
func TestParseDateTimeInvalid(t *testing.T) {
	_, err := ParseDateTime("2026-01-01")
	if err == nil {
		t.Fatal("expected error")
	}
}
