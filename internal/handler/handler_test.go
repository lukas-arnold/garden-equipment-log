...
func TestTemplateFuncsFormat(t *testing.T) {
	...
	formatFloat := funcs["formatFloat"].(func(float64, int) string)
	formatEuro := funcs["formatEuro"].(func(float64) string)
	formatTime := funcs["formatOperationTime"].(func(float64) string)

	if formatFloat(12.345, 2) != "12,35" {
		t.Fatal("formatFloat failed")
	}

	if formatEuro(19.5) != "19,50 €" {
		t.Fatal("formatEuro failed")
	}

	if formatTime(125) != "2 h 5 min" {
		t.Fatal("formatOperationTime failed")
	}
}

func TestFormatOperationTimeHoursOnly(t *testing.T) {
	funcs := getTemplateFuncs()
	format := funcs["formatOperationTime"].(func(float64) string)
...
func TestRenderTemplate(t *testing.T) {
	...
	h.renderTemplate(rec, "templates/device/add.html", nil)
...
func TestRenderTemplateWithData(t *testing.T) {
	...
	h.renderTemplate(rec, "templates/device/edit.html", data)
...
func TestRenderHistoryTemplate(t *testing.T) {
	...
	h.renderHistoryTemplate(rec, "templates/device/history.html", nil)
...
func TestRenderHistoryTemplateWithData(t *testing.T) {
	...
	data := struct{ Name string }{Name: "History"}
	h.renderHistoryTemplate(rec, "templates/device/history.html", data)
