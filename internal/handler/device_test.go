...
func TestHandleAddDeviceGet(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/device/add", nil)
	rec := httptest.NewRecorder()

	h.HandleAddDeviceGet(rec, req)
...
func TestHandleAddDevicePost(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/device/add", strings.NewReader(form.Encode()))
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	rec := httptest.NewRecorder()

	h.HandleAddDevicePost(rec, req)
	...
	if len(devices) != 1 || devices[0].Name != "Mower" {
		t.Fatal("device was not added correctly")
	}
}

func TestHandleEditDevice(t *testing.T) {
	h := testHandler(t)
	h.storage.AddDevice(models.DeviceInput{Name: "Mower"})
	devices, _ := h.storage.GetDevices()

	req := httptest.NewRequest("GET", "/device/edit", nil)
	req.SetPathValue("id", strconv.FormatInt(devices[0].Id, 10))
	rec := httptest.NewRecorder()

	h.HandleEditDevice(rec, req)
...
func TestHandleSaveDevice(t *testing.T) {
	h := testHandler(t)
	h.storage.AddDevice(models.DeviceInput{Name: "Old"})
	devices, _ := h.storage.GetDevices()
	...
	req := httptest.NewRequest("POST", "/device/save", strings.NewReader(form.Encode()))
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	req.SetPathValue("id", strconv.FormatInt(devices[0].Id, 10))
	rec := httptest.NewRecorder()

	h.HandleSaveDevice(rec, req)

	if rec.Code != http.StatusFound {
		t.Fatalf("expected status Found, got %d", rec.Code)
	}

	updated, _ := h.storage.GetDevices()
...
func TestHandleSaveDeviceInvalidID(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/device/save", nil)
	req.SetPathValue("id", "abc")
	rec := httptest.NewRecorder()
...
func TestHandleDeleteDevice(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/device/delete", nil)
	req.SetPathValue("id", strconv.FormatInt(devices[0].Id, 10))
	rec := httptest.NewRecorder()

	h.HandleDeleteDevice(rec, req)

	if rec.Code != http.StatusFound {
		t.Fatalf("expected status Found, got %d", rec.Code)
	}

	result, _ := h.storage.GetDevices()
	if len(result) != 0 {
...
func TestHandleDeviceHistory(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/device/history", nil)
	req.SetPathValue("id", strconv.FormatInt(devices[0].Id, 10))
	rec := httptest.NewRecorder()

	h.HandleDeviceHistory(rec, req)
